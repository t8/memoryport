import { useState } from "react";

interface Props {
  onNext: (ucApiKey: string | null) => void;
}

export default function StepApiKey({ onNext }: Props) {
  const [key, setKey] = useState("");

  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-xl font-bold">Cloud backup (optional)</h2>
        <p className="text-zinc-400 text-sm mt-1">
          Back up your memory permanently to Arweave. Requires a Pro subscription.
        </p>
      </div>

      <div>
        <label className="block text-sm text-zinc-400 mb-1.5">Memoryport Pro API Key</label>
        <input
          type="text"
          value={key}
          onChange={(e) => setKey(e.target.value)}
          placeholder="uc_... (from memoryport.ai/dashboard)"
          className="w-full px-3 py-2 bg-zinc-800 border border-zinc-700 rounded-md text-sm focus:outline-none focus:border-zinc-500"
        />
        <p className="text-xs text-zinc-600 mt-1">
          Get a key at{" "}
          <a
            href="https://memoryport.ai/dashboard"
            target="_blank"
            rel="noopener"
            className="text-zinc-400 underline"
          >
            memoryport.ai
          </a>
        </p>
      </div>

      <div className="flex gap-3">
        <button
          onClick={() => onNext(null)}
          className="flex-1 py-2.5 border border-zinc-700 hover:bg-zinc-800 rounded-md text-sm font-medium transition-colors text-zinc-300"
        >
          Skip — local only
        </button>
        <button
          onClick={() => onNext(key.startsWith("uc_") ? key : null)}
          disabled={!key.startsWith("uc_")}
          className="flex-1 py-2.5 bg-emerald-600 hover:bg-emerald-500 disabled:opacity-30 disabled:cursor-not-allowed rounded-md text-sm font-medium transition-colors"
        >
          Save key
        </button>
      </div>
    </div>
  );
}
